...
<span style="font-size:24px">`r "\U1F4CB"`</span>: 高鹽、高脂食品的個數
```{r}
table(USDA$HighSodium, USDA$HighFat)
```
<br>

<span style="font-size:24px">`r "\U1F4CB"`</span>: 高蛋白的食品中鐵質的平均值(去除遺漏值)
```{r}
tapply(USDA$Iron, USDA$HighProtein, mean, na.rm=TRUE)
```
<br>

<span style="font-size:24px">`r "\U1F4CB"`</span>: 高碳水化合物的食品中含有維他命C的最大值(去除遺漏值)
```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, max, na.rm=TRUE)
```
<br>

<span style="font-size:24px">`r "\U1F4CB"`</span>: 高碳水化合物的食品中涵以維他命C的總覽(去除遺漏值)
```{r}
