...
pred_tr = predict(glm2, newdata=TR, type="response")
pred_ts = predict(glm2, newdata=TS, type="response")
```
...
```
<br>
<br>&emsp; 【C】 分別算出`Training`和`Testing`的`ACC`、`SENS`和`SPEC`
```{r}
trCMX = table(Acture=TR$PoorCare, Predict=pred_tr > 0.5)
tsCMX = table(Acture=TS$PoorCare, Predict=pred_ts > 0.5)
sapply(list(Train=trCMX, Test=tsCMX), A2x2)
```
