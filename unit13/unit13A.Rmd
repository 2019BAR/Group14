...
```
Ans:OfficeVisits：1.0856, Narcotics：1.0793
```
&emsp; &emsp; ■ 兩個自變數的機率效果分別是？<br>
```
Ans:OfficeVisits：1.0753, Narcotics：1.0698
```
####【code】
```{r}
...
predict(glm1, df, type="response") #0.12035 

0.12035/0.11192 #OfficeVisits機率效果

df = data.frame(OfficeVisits = quantile(D$OfficeVisits,0.25), Narcotics=quantile(D$Narcotics,0.25)+1)
predict(glm1, df, type="response") #0.11973
...
predict(glm1, df, type="response") #0.30541

0.30541/0.28947 #Narcotics機率效果

```

&emsp; ■ 比較以上兩個題目的答案，我們可以觀察到什麼？
```
Ans:機率效果會隨著變數變大而變小，勝率效果則不會改變。
```
<br><br>
</p class="qiz">
