...
```{r}
logit2 = sum(b * c(1, 2, 3))
odd2 = exp(logit2)
prob2 = odd2/(1+odd2)
c(logit=logit2, odd=odd2, prob=prob2)
```
<br>
...
##紅點y=0、綠點y=1
##藍線代表logit=0，y=1的機率為0.5，線上方代表模型預測y=1的機率越大
```
...
```{R}
#，而越上面的線機率會越低；越下面的線機率會越高
#九條線代表模型預測那條線上y=1的機率,分別為p=0.1~0.9,深藍色代表p=0.5
```

<span style="font-size:24px"> `r "\U1F5FF"` : </span>
Given any point in the figure above, how can you tell its (predicted) probability approximately?<br>
```{R}
#只要知道x1.x2的值，即可利用邏輯式函數求得任意點y=1的機率
```
...
```{R}
#logit: 利用X做成的一個線性組合
#logistic function: 將線性組合的值(logit)代入連結函數( 1/(1+exp(-g(x))) ) 以預測出y=1的機率
```
